...
import { ChemistryKnowledgeBase } from './services/ChemistryKnowledgeBase';
...
function App() {
  ...
  const handleSearch = async () => {
    ...
    if (LocalFallbackService.isRateLimited()) {
      // Use Chemistry Knowledge Base if available
      const knowledgeBaseResponse = ChemistryKnowledgeBase.getResponse(question);
      setAnswer(knowledgeBaseResponse);
      return;
    }

    // Sanitize input to properly handle special characters
    const sanitizedQuestion = encodeURIComponent(question.trim());
    ...
    try {
      const response = await ollamaService.generateResponse(sanitizedQuestion);
      setAnswer(response);
    } catch (error) {
      if (error.message.includes('rate limit exceeded')) {
        LocalFallbackService.setRateLimited();
        // Use Chemistry Knowledge Base
        const knowledgeBaseResponse = ChemistryKnowledgeBase.getResponse(question);
        setAnswer(knowledgeBaseResponse);
      } else {
  ...
  // Debounce the search handler to prevent rapid-fire API calls
